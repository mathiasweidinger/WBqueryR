...
#'
#' @return This function does not return any element.
#' @export
#' @import RSelenium netstat
#'
#' @examples
#'\dontrun{
#'WBdownload(item.id = 1234,
...
WBdownload <- function(
    ...
    passw$clickElement()
    passw$sendKeysToElement(list(user.password))
    ...
    if(length(remDR$findElements(using = 'xpath',
                                 '//textarea[@id="abstract"]'))!=0){

        # find abstract
        abstract <- remDR$findElement(using = 'xpath',
                                      '//textarea[@id="abstract"]')
        # find check box
        agree <- remDR$findElement(using = 'xpath',
                                   '//input[@id="chk_agree"]')
        # find submit button
        submit <- remDR$findElement(using = 'xpath',
                                    '//input[@id="submit"]')
        # find body
